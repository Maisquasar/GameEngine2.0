...
void Resources::Mesh::BillboardUpdate(Math::Mat4 MVP, std::vector<Resources::Material*> materials, const Math::Vec2& _size, bool outline)
{
	if (!Loaded)
		return;
	if (!_initialized)
...
void Resources::Mesh::BillboardPicking(Math::Mat4 MVP, std::vector<class Material*> materials, const Math::Vec2& _size, int id)
{
	if (!Loaded)
		return;
	if (!_initialized)
